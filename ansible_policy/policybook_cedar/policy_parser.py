...
import ansible_policy.policybook_cedar.policybook_models as pm
from ansible_policy.policybook_cedar.condition_parser import (
    parse_condition as parse_condition_value,
)

VALID_ACTIONS = ["permit", "forbid"]
...
def parse_policies(policies: Dict, vars: Dict) -> List[pm.Policy]:
    ...
    for pol in policies:
        ...
        non_condition = pm.Condition(
            "",
            [],
        )
        if ("condition" not in pol) and ("exception" not in pol):
            parsed_pol = pm.Policy(
                name=name,
                condition=non_condition,
                exception=non_condition,
                actions=parse_actions(pol),
                enabled=pol.get("enabled", True),
                tags=tags,
                target=target,
            )
        elif ("condition" in pol) and ("exception" not in pol):
            parsed_pol = pm.Policy(
                name=name,
        ...
        elif ("condition" not in pol) and ("exception" in pol):
            parsed_pol = pm.Policy(
                name=name,
                condition=non_condition,
                exception=parse_condition(pol["exception"], vars),
                actions=parse_actions(pol),
                enabled=pol.get("enabled", True),
                tags=tags,
                target=target,
...
def parse_condition(condition: Any, vars: Dict) -> pm.Condition:
    if isinstance(condition, str):
        ...
        if len(condition) == 1 and keys[0] in ["any", "all"]:
            ...
            raise Exception(f"Condition should have one of any, all: {condition}")
    else:
